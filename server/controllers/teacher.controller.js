...
export const login = async (req, res) => {
  try {
    ...
    if (!username || !password) {
      console.log(`[Teacher Login] Missing credentials`);
    ...
    console.log(`[Teacher Login] Teacher found: ${teacher ? 'Yes' : 'No'}`);

    if (!teacher) {
      console.log(`[Teacher Login] Teacher not found: ${username}`);
      return res.status(401).json({ message: "Invalid credentials" });
    }

    console.log(`[Teacher Login] Comparing password...`);
    const isPasswordValid = await bcrypt.compare(password, teacher.password);
    console.log(`[Teacher Login] Password valid: ${isPasswordValid}`);

    if (!isPasswordValid) {
      console.log(`[Teacher Login] Invalid password for: ${username}`);
      return res.status(401).json({ message: "Invalid credentials" });
    }
    ...
    console.log(`[Teacher Login] Success for: ${username}`);
...
export const stopAttendance = async (req, res) => {
  try {
    ...
    const stopped = await stopPythonScript(students, teacher.subject);
    isCapturing = false;
    io.emit("capture-status", { capturing: false });
    res.json({
